...
The TypeScript products build from a package-lock.json, so a release ships the exact dependency tree we tested. From September 2026 on, every release of the Forge apps and the VS Code extensions runs npm audit against the npm advisory database, and a moderate or worse advisory in a shipped dependency holds the release until the dependency is fixed; if we ever ship past such an advisory because the code it names is not reached, the release notes say so. The JetBrains plugins name their direct Java libraries by exact version in the Gradle build (the Parquet readers, the SQLite and DuckDB engines). We have no advisory scanner on that side yet; a daily job of ours watches the release feeds of the Parquet, Arrow and DuckDB projects and rings when a new release lands, and every release passes the JetBrains plugin verifier on the oldest and newest IDE builds the plugin declares.
...
- The Forge apps declare two third-party packages, @forge/bridge and fflate. The engine code compiled into each app carries the readers it is built from (hyparquet, flechette, fzstd and lz4js), and the licenses.txt shipped inside the app lists every one of them with its license; the lock file lists what each package brings in.
