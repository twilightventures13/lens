...
Export CSV writes the open table as one download, with a progress line and Cancel in the status bar. It covers the open file, the open sheet of a workbook, or a log as level, timestamp and message columns; with a chip set on, a log exports the lines the set keeps. The file is assembled in the page and nothing leaves the browser.
